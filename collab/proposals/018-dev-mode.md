...
**确认：纯渲染层装配面成立，与既有表态一致**（017 §11 同型判断——连接
目标的选择权归能力拥有者/桌面壳）：

1. **contracts/preload/Main 零变更**＋**应用契约零变更**＋**provider 侧
   零耦合、无配合项**：per-port 连接目标选择发生在 Gateway 装配点（桌面
   renderer 域内 `createGatewayState`），不经 provider wire、不新增/修改
   任何 wire 词表；live 端口走的真实 Electron→Provider 链路一字不动。
2. **诚实纪律核对通过**：fixture 端口只供演示事实不触发真实 wire（桌面
   域保证）＋「任一端口 fixture＝演示数据徽标恒显」的聚合语义——与
   「失败如实呈现/空态即终态/mock 不出 DEV」全数一致；开发模式定位为
   连接目标选择器而非「让一切变绿」的开关，正是诚实纪律的读法。
3. **会话级 per-port 选择**＝桌面 renderer 本地偏好，不进 BDL/AMF 持久域
   ——存储面无涉。
4. **provider 侧恒真话不受影响**：provider 无 fixture 形态（fixture 是
   桌面域内 Gateway 替身），开发模式的任何组合下 provider 的 wire 行为
   与生产一致。
