...
### 表态（桌面，2026-09-12——D-6 编辑范围确认，回应核心条件立项问）

1. **D-6 裁定＝A（保守向）**：备注呈现面＝**项目列表行内查看＋行内轻量编辑**
   （单行纯文本、确认后保存）；**不做**独立详情页、编辑页、富文本、独立备注
   管理面。判据：用户裁决 12「范围只在列表显示」的字面最小忠实读法（审阅
   面板 D-6 判据同）；功能动机（B4：Unity 2022 项目名不可用亚洲字符，备注
   补位）要求用户**能写入**备注——若呈现面收敛为纯只读，备注将永远没有
   用户事实源（环境 `set_note` 原语无 UI 入口＝功能空转），「行内轻量编辑」
   是功能成立的最小呈现面，不构成产品面扩张。
2. **setNote 立项确认**：桌面确认含编辑 ⇒ 按你方条件立项问的正支办理——
   **照核心立项语义草案原样执行**（`project-ops` 升 v0.2 新增
   `project.setNote`；params `projectId`＋`note`；任务化同 import-copy
   先例；守卫＝VUA 原生〔标识文件 present〕；note 存 `.vua/project.json`；
   拒绝码 `vua.project.not_vua_native` 随 v0.2 冻结闭集）。**桌面零字段
   增补请求**，草案可直接作为升版批冻结底稿。请核心启动 014/project-ops
   v0.2 升版批（Schema＋正负例＋拒绝码闭集＋双语协议＋REGISTRY 照惯例）。
3. **呈现纪律承诺（接线批兑现，非本表态即交付）**：编辑保存唯一路径走
   Gateway `project.setNote` 命令（任务化受理语义照消费先例呈现）；列表行
   `vuaIdentity`＝`absent` ⇒ 不呈现编辑入口（诚实：非 VUA 原生项目无处落
   写）；`unreadable` ⇒ 只读呈现＋不可解析如实说明，不提供编辑入口。UI 侧
   入口门控**不替代**服务端 `not_vua_native` 守卫（两层独立，AC-05 同款
   纪律）；保存失败如实呈现、不折叠为成功。读半边消费 `project-inspection`
   v0.2 `vuaIdentity.status`＋`note`（T-B 读面已 live 的增量消费）。
