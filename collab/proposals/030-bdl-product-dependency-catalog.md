...
### 裁决登记（集成代录，2026-09-22 第 166/167 批同窗——向量文件形态竞速，操作者裁）

第 166 批同窗竞速事实：产线席（wt-4）冻结批将向量文件形态落位为
`schemas/bdl/v0.2/vectors/` **JSON 形态**（恰 17 向量，P1–P9＋N1–N8），数据席
（wt-5）同窗表态提出**第三案**（`schemas/bdl/v0.2/examples/` SQL 片段例集，
「向量实例形态跟随权威语言」原则）。两案撞窗，操作者裁决（集成代录，原文）：

> **裁：维持冻结 JSON 形态**（与仓内 production v0.2 vectors 先例一致，闭集本体
> 已冻结不重开），数据域第三案登记 030 备查不作采纳，数据域 v0.5 实现时如遇形态
> 硬阻力走勘误批再议。

集成执行：①冻结 JSON 形态维持——`vectors/` 17 文件不动，冻结词面不因本裁决
变化；②数据席第三案（本线程上一节）**登记备查、不作采纳**，其「跟随权威语言」
原则与文件契约设计作为 v0.5/勘误批的参考材料留档；③如数据域 v0.5 实现环遭遇
...
### 回复（产线/wt-4，2026-09-23 第 176 批——提取管线 1.5.0 重新规格化注记）

**注记性质**：操作者第 176 批派单＋TICK v1.7 领单前核对义务（BOARD「最新裁决
（2026-09-22）」节：一切旧结论、旧 proposal 与 1.5.0 冲突处以 1.5.0 为准）——
「030 提取管线切片」候派中，其原验收口径按 `docs/product-boundary_ZH.md`
**1.5.0** BDL 节重新规格化后方可领单。本注记即核对结论落档；**零代码、零
schema、零测试触发、零 BOOTH 访问**。

- **① 定位重定＝实验功能，旗标默认关**：本提案 §2「提取管线方向」与执行序
  建议第 2 步的实现物，在 1.5.0 语境下＝「自动兼容性证据收集」，**实验功能、
  默认关闭**；关闭自动收集不得影响 BDL 基础存储、普通导入与 Recipe 来源补充
  （1.5.0 原文律）——依赖库既有职责面（基础存储/素材身份/来源关联/目录）不
  受旗标约束。旗标本体与旗标 UI 候新提案（见⑤），不在已落库面内。
- **② 输入源重定＝「用户实际的 BOOTH 浏览和 Unity 使用过程」**：原 §1 取证
  所对应的「产品页自动抓取面」自本注记起**降级为候选来源之一，设计留白**——
  不预设实现、不随建库环实施；「不恢复全站云端采集方向」钉死（§4 红线照旧）。
  §1 调查本身不作废：其版面惯例结论（自拟小节标题/版本钉行/列点/单行声明）
  仍是保守提取模式与冻结词面（extraction_method 六值闭集）的证据原型，只是
  「自动化抓取管线」这一实现方向候重议。§5.5（公开面覆盖缺口）／§5.6（U18
  终裁联动）未决项维持开放。
- **③ 保守提取原则维持，与冻结面零冲突**：结构化版面模式／宁缺勿猜／一切提
  取行默认未确认线索（confirmed_by_human 默认 0＋confirm_dependency_resolution
  唯一确认写入者）——与 BDL v0.2「线索非结论」律、bdl-queries v0.5 匹配规则
  v1（零子串零模糊零等价、包名形诚实空集）＋advisory 规则 v1 双门完全同向；
  1.5.0「没有证据代表未知，不能当作兼容或依赖已解决」与 v0.5 空态诚实律
  （total:0＝无匹配名义≠无此依赖）互为印证。
- **④ 已落冻结面与已验收环不受影响**：BDL v0.2 schema＋17 向量＋协议本
  0.2/0.2.1、bdl-queries v0.5 FROZEN、store v0.2 落库、dependencies.* v0.5
  真实执行器环（本提案查询族「冻结→落库→接线→TS 面→真实执行器」五环闭环）
  ——1.5.0 明记本节「为已接受产品方向而非实现或真机验收声明、冻结协议与数据
  格式因此不自动改变」。重新规格化只及**未实现的提取管线切片**的验收口径；
  已冻结词面零变化、已验收环零重开。
- **⑤ 抓取面与旗标 UI＝候新提案，不随 030 建库环实施**：证据采集面涉用户实
  际浏览/使用过程的观察通道（跨桌面/Unity/数据域）与实验旗标 UI，按域纪律候
  新提案立项后另行办理；提取管线实现切片若开窗，输入源接线按届时已立提案办
  理，不默认继承 §1 的自动化抓取形态。§2「来源＝AMF 已验证的来源观察（现有
  G13 写路径管道的延伸）」照旧成立——该管道系用户实际使用过程中产生的来源
  观察，与 1.5.0 输入源定义相容。

### 回复（数据/wt-5，2026-09-23 第 176 批——产品边界 1.5.0 数据域对账：基础存储保留判定＋来源补充落点登记＋依赖降级路径数据视角）

操作者第 176 批指派＝TICK v1.7 领取前核对义务兑现：实读 main 权威正文
`docs/product-boundary_ZH.md` 1.5.0（「素材来源（分享时再补，尽量少填）」
节＋「BDL」节＋「待验证与未决」五项）后，对本提案已建面作数据域对账。
**纯注记批：零代码、零 schema、零协议词面变化；只登记，不实施；待验证五
项无一项标为已解决。**

- **① 基础存储保留判定＝完全存活，零废弃**。1.5.0 BDL 节明文「保留已存在
  的基础存储、素材身份、来源关联与目录能力」，本提案已建链恰为该保留面
  的承载：BDL v0.2 冻结（产线第 166 批）＋store v0.2 落库（产线第 168 批）
  ＋bdl-queries v0.5 查询族冻结（数据第 168 批，§5.7 案 A）＋核心接线环
  ＋TS 面环（核心第 171 批／桌面第 172 批）＋真实读执行器（产线第 174 批，
  装配翻真）。「自动兼容性证据收集为实验功能，默认关闭」在 1.5.0 中只约
  束**提取管线**——即本提案执行序 2 尚未开工的「保守提取＋人工确认面」
  切片，其验收口径候产线域按 1.5.0 重新规格化（集成留言已明示领单前核
  对）；该约束不触及上列已建五环中的任何一环。1.5.0 同句「关闭自动收集
  不能关闭基础存储、普通导入或 Recipe 来源补充」与「没有证据代表未知」
  分别由既有事实承载：v0.2 持久面（products／observations／download_
  events／warehouse 诸表）与 v0.5 诚实空态（total:0＝无匹配名义非无此依
  赖）本就是按此语义建的。
- **② 来源补充「分享时再补」的数据面落点登记（只登记，不实施）**：
  - **已确认关联记忆**＝`confirmed_by_human=1` 行的复用律——1.5.0「已
    确认关联应记住，避免重复劳动」在数据面的现状承载已存在：
    `confirm_dependency_resolution` 唯一写路径（产线第 168 批落库）＋
    lookup confirmed-only 读出（resolvedProductId 仅对已确认消解出线）。
    关联记忆无需新表新面；候选→确认工作流（人工点选后翻 1）是实施面，
    候切片指派。
  - **已购库点选／关键词候选的数据源方向**（现状盘点，非设计承诺）：
    本地已购/持有事实在 bdl.db 已有三张面——`download_events`（点击上
    下文 `initiated_from_page_url`／`url_chain`／`suggested_file_name`，
    download-events v0.1 冻结闭集）＋`warehouse_items`（kind=
    imported_material|downloaded_material）＋`local_artifacts`（内容身
    份 sha256 与检查态）；关键词候选的投影基础＝`products.search_text_
    normalized`（v0.2 冻结列，只读可用）。以这些面为输入做「点选/搜索
    候选列表/批量关联」属未来工作流设计，本注记零实施、零词面新增。
  - **「尽量少填」对消费面无字段级影响**：lookup/listByProduct 词面零
    变化——来源补充发生在写入/确认侧，不在查询侧；v0.5「线索非结论」
    律（未确认线索仅在 listByProduct 原样贴标、lookup 恒不出线、null
    不透露无解析/未确认之分）与 1.5.0「BOOTH ID 属于作者或用户的来源
    声明，不是 VUA 对真实性、未修改状态或可复现性的认证」天然同律，
    零冲突零补丁。
- **③ 依赖降级路径（1.5.0 待验证五项之一）——数据视角一句落点**：取证
  关闭（提取管线不启用）时，dependency_observations 无行，lookup 返回
  total:0 诚实空集——这已是现行为（执行器空库诚实空集测试钉死，产线第
  174 批）；而 1.5.0 未决项「自动取证关闭或解析失败时**如何完成依赖补
  齐**，仍需明确实际流程」属流程设计，候新提案，本注记不设计、不代决，
  该项维持未决。
### 回复（核心，2026-09-23 第 176 批——1.5.0 对账注记：依赖降级路径核心域落点登记）

应操作者第 176 批指派（product-boundary 1.5.0「制作范围与产品裁决」核对义务，
本提案侧对账；纯 collab 注记批，零代码零冻结面变更零测试触发）：

- **依赖降级路径核心域落点一行登记（1.5.0 未决项原文；不立项不实施，候用户排
  优先序）**：取证关闭或解析失败时的依赖补齐流，其**应用用例/编排面落核心域**——
  向上骑 dependencies.* v0.5 已冻结建议面（「无证据＝未知」「建议非事实」「空态
  ＝无匹配名义非无此依赖」三律已随 v0.5 协议本冻结）与产线座确认写动作面
  （confirmed_by_human 唯一写路径），接「检测→反查→补装/指引」用例编排与诚实
  未知呈现；实际流程形状候 1.5.0 未决项解冻与 U18 终裁，本登记仅钉落点，不预决
  方法面。
- **1.5.0 对本提案已冻结面无追溯影响确认（一句）**：2026-09-22 勘误把供数与分享
  拆开两层——本地依赖供数守 BDL 契约、不受分享边界限制；dependencies.* v0.5
