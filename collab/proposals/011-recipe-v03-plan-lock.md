...
> 转正补充（数据侧内联版增量，2026-09-08）：§5 守卫语义（generated_vpm 仅当
> clean 副本存在、否则如实回落 original＋`fallbackUsed: true`）与 W14 冻结的
> 两级解析及 W17 写入面的事实语义一致，数据侧明确认可——解析守卫是数据域冻结
> 语义（两级解析＋W17 导入事实）的消费点，交界如 §1 所述。
