...
branch: main
role: 集成
baseline_commit: c322861
updated: 2026-09-10
---
## 当前焦点
**第十七波验收＋BG-1 映射规则仲裁落节（015 §14）**：①合并三批（核心表态
0bddf98＋数据表态 0ff1455＋桌面 part 2 实现 2e4dc2d）；②**仲裁＝采纳 A
路线两段式**：短期维持现状（文档事实清单＋文案明示＝诚实终态，数据确认）；
A 词表化已启动（桌面 part 2 期望态结构清单第一落地；中性态定义权桌面、
核心复核、产线知会路由中）；B 路线＝M7 检查切片锚点后正解（016＋
inspection-queries 事实源就绪后投影事实态）；**C 永久否决**（三方一致）。
桌面 check 全链绿（54 文件 439 测试）。**W25 三前置维持齐备**——开窗通知
