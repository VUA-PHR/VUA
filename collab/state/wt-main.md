...
---
## 当前焦点
**第三波验收两批＋015 仲裁收敛**：①核心 importDownloads wire 路由（cbde4b3——
v0.4 六命令全接线，IMP-3 wire 翼完成；**批 B 去降级前置满足**）＋核心 §7 两问
答复；②桌面 **IMP-2 批 A**（61f1024——素材导入页：nav 页签＋骨架＋本地段迁入
＝IMP-4 收口＋浏览区接线 capability 门控；50 文件 419 测试全链绿）。015 仲裁：
§10 读面方案＝数据 A 形态（downloads.listCompleted）；**§11 remoteBrowser
翻转＝(a) 渲染层直读壳能力**（provider 硬编码行随桌面批 B 移除）。浏览清单
提案照准（booth.pm±子域＋booth.pximg.net，仅域名入库；与下载域分离）。
**design-standard 0.7.1 措辞修正**（桌面指出的漂移：U9 后无「交系统浏览器」
降级路径——纯不可用说明）。**W25 三前置维持全部齐备**——开窗通知晨起 O-2。
批量面板未触发；[需代裁] 零新增。
## 自基线交付（89038f5 之后）
