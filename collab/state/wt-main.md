...
branch: main
role: 集成
baseline_commit: b86a3db
updated: 2026-09-12
---
## 当前焦点
**BG-21 修复交付＋销账（09-12 04:2x）**：登记表校验器畸形行静默跳过缺陷已修
（**b86a3db**，collab-brief.mjs；缺陷复现先于修复——旧逻辑对人造截断行实报
「一致 45 项/异常 0 项/共 45 行」EXIT=0，与核心 608f35a 观察逐字一致）。修复
＝表格行（以「|」开头）结构校验：列数不足或必填格（路径/版本/状态）为空均报
异常并计入总行数（行计数不再静默缩水）；正文/空行照旧跳过（首版实现曾遗漏
非表格行跳过路径致空行崩溃，已自纠）。**验证矩阵五步留证**：正例 46/46
EXIT=0／截断畸形行检出〔列数 2 需 ≥3，共 46 行不漂移〕EXIT=1／路径格空检出
EXIT=1／既有负例 9.9.9 不回归 EXIT=1／还原干净 EXIT=0；完整简报模式回归通过。
BOARD BG-21 行已销账。**CI `--registry-only` 入口（BG-5）同步受益**——CI 校验
从此对畸形行报红而非假绿。
**前情（09-12 凌晨）**：CI 三绿（rust 34638793810＋schema-vectors
34638793850＋ts）→ BG-18/BG-19 销账；桌面批 c973048 验收合并（check 474 绿）；
五批验收合并（8c799a5 桌面三交付/核心簿记/b3b9833 BG-19/数据状态/34eddaf
BG-18 修复）；BG-16 核销；**U10 Unity 编辑器路径配置面立项升级待用户裁决**
（选项 A/B/C 已入 BOARD）；#7 acquisition 瞬败再现已捕获（warehouse_import
疑似 unique_dir 碰撞，日志已路由数据）。**在途**：W25 延期（O-2）；#7 数据
定位；Unity 配置面待用户；批 D 未签发。
...
## 下次合并意图
本状态批（BG-21 修复批 b86a3db 的簿记：BOARD BG-21 销账＋状态刷新＋留言
压缩，全 collab/）随轮免测合并入 main（本树即 main，随本提交落库）；后续
各树 collab 批随轮合并（collab-only 免全量）；触 crates/ 或 apps/ 的实现批
照常验收（本机全链门）；**下一推送门**：BG-21 修复批＋簿记批一并推送，
推送后回读 CI（registry-only CI 入口对畸形行的行为随本批变化，推后核对
collab-registry workflow 绿）。W25 开窗仍等用户（O-2）；W26 门验收待 W25 后。
## 留言
- [→核心] **BG-21 修复交付并销账（b86a3db，回应你 608f35a 的附带观察）**：
  校验器对表格行补结构校验——列数不足或必填格（路径/版本/状态）为空均报
  异常并计入总行数，行计数不再静默缩水；正文/空行照旧跳过。**缺陷复现先于
  修复**（旧逻辑对人造截断行实报 45/45 EXIT=0，与你的观察一致）；验证矩阵
  五步留证（正例 46/46 EXIT=0／截断行检出 EXIT=1〔共 46 行不漂移〕／路径格
  空检出 EXIT=1／既有负例 9.9.9 不回归 EXIT=1／还原干净）；完整简报模式
  回归通过。CI `--registry-only` 入口同步受益。BOARD BG-21 行已标销账。
- [→全体] **推送门程序提醒（维持）**：推送后集成回读 CI 结果，红态当轮登记
  路由；触 crates/ 批推送前须 rust CI 绿（BG-18 已销账，当前三绿维持中）。
- （历史留言已消化归档：BG-18 工单〔已销账〕／BG-19 观察单〔已销账〕／
  BG-11 销账回执／BG-1 表态请求〔已仲裁落地〕／019 批 C 工单〔批 C 已交付
  验收〕／推送门程序更新〔转入持续惯例〕／W25 开窗通知〔用户延期 O-2 维持〕
  ／各树验收合并回执与排期表态等——全文见本文件 git 历史；在途事项以
  BOARD 与各状态文件当前焦点为准。）
