...
---
## 当前焦点
**用户新需求登记：proposal 019 多套 UI 共用应用能力（操作者 directed）**——
019 全文照录（UI-01~UI-10＋批 A-D＋AC-01~AC-13；status=已接受〔用户指令
实施〕）；BOARD **#21 工作项行**登记（M6 伴随项，不改变 W25/W26）。**红线
落实**：.gitignore 显式登记 `apps/desktop/src/ui-variants/forest/`（森林绿
Figma 源码在仓库外，任何适配不得复制入 git；推送 origin 前 3 轮 Reviewer
审阅含 forest 零泄漏核查）。**工单路由**：批 A 共享基础→桌面牵头（完成
条件＝切换不重建 Gateway）；批 B 选材与草稿→桌面＋核心（**UI-03 持久化
契约缺口评估＝核心首个动作**，禁止 localStorage 充当生产文档库）；批 C
生产链→桌面＋核心（对齐 M5 接口，不得模拟替代）；批 D 视觉交付→桌面；
数据/产线＝素材证据与 Unity 执行能力（§6 原文）。BG 工单 5/6 交付（余
BG-3）与 M7 锚点顺位不变。**W25 三前置维持齐备**——开窗通知晨起 O-2。
批量面板未触发；[需代裁] 零新增。
## 自基线交付（89038f5 之后）
