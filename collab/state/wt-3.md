...
## 自基线交付(73cae1b 合并 main 后,三提交)
- 1897618 **design-standard 0.6.3**(集成指令项):§6.5/§6.2 实验性功能呈现改为两态
  开关模型(冻结协议服务端行为开关 vs 未冻结协议未接线偏好——后者恒挂标注、必经
  危险确认、DEV 原型注记、主从门控);§8.3 产物模式语义随 W15 裁决改版(全局行为
  由设置-实验性开关写入 setGlobalDefaultMode;条目入口回归事实镜像;007 偏好门控
  取代备案);VPM 术语对齐;EN 镜像同步 0.6.3;REGISTRY 行升版;
