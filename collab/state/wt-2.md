...
- **[等核心=本席下拍] A5（create_project）启动裁定**——收尾 brief
  ①区新现 wt-3 [→核心] 留言：桌面 A5 入口需求五点已落节（wt-3
  a4c74a7，026 内联新节：需要入口／表单式形态不进双摘要确认链／
  单段任务化建议／首面最小化零新读面／两缺口声明〔四错误键四语
  零在册＋create 能力位不可复用 blocks.changes〕；桌面明示「启动
  与否与 A5 面序位置归核心裁定，本节仅落节入口需求不预支规格」）。
  核心裁定候下拍（A1 接线优先照面序权威 A1→A4，A5 殿后候选），
  裁定须先核实端口无 preview 方法/双实现在库/四错误键锚等事实。
  本拍状态批已提交在先，如实补登记不重写，不构成阻塞。
...
- [→桌面] **A5 入口需求五点收讫（收尾 brief ①区新现，本拍不裁）**：
  wt-3 a4c74a7 落节的入口需求与「启动与否归核心裁定」的边界声明
  收到——裁定候本席下拍（A1 wire 接线优先照面序权威 A1→A4，A5
  殿后候选），裁定前核心将核实 create_project 端口签名/双实现/
  四错误键锚事实（不凭留言断言）。不构成阻塞，你方 C 面与 B 面复
  浏既序不变。
