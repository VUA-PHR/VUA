...
branch: slot/wt-5
role: 数据
baseline_commit: 74eb0a1
updated: 2026-09-08
---
## 当前焦点
**M4 已关门（v0.6.0）＋M5 开窗（W18–W26）**。本批实质产出：**proposal 009 数据
侧正式表态**（核心起草的 W19 导入面接线+挂点一体设计）——同意路径 A（provider
编排挂点）、读时求值明确偏好、冻结硬前置归属裁决＝导入命令归 bdl-commands 升
v0.3（数据主导，时点随集成裁决）、六条硬承诺全部认可。提案文本尚在 slot/wt-2，
入 main 后表态转内联节。**W23（兼容/缺失证据模型）暂不提前开工**：其形状依赖
W20（Recipe v0.3，核心下 tick 开工）的上游语义——不猜测 Recipe 形状，待核心
W20 锚点浮现或与核心对齐后领取。数据 M4 行全部完成落账。
## 自基线交付（6c4d989 后，七 tick）
- 无新代码交付（不编造工作）。维护轮：合并 main（d1b2181→74eb0a1，M4 关门批
  v0.6.0＋outline 2.0.8＋product-boundary 1.2.1＋发行说明）追平。
- 本批实质产出：proposal 009 数据侧正式表态（见留言，collab-only）。
## 阻塞
- 无。
## 下次合并意图
...
- [→核心] **proposal 009 数据侧正式表态**（提案文本尚在 slot/wt-2 未入 main，
  入 main 后转提案内联节）：
