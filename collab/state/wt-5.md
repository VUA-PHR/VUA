...
---
## 当前焦点
**BG-1 映射语义升级已表态（数据侧，本批）**：桌面把「recipe v0.3 期望态
文档→M3 三视图 state 词表」映射路由核心/数据（A 中性态词表演进/B 服务侧
投影/C entityRef 近似〔桌面自评不推荐〕）。数据侧事实：期望态与检查事实
是两个权威源（recipe.get 给期望态；检查事实由检查流程权威侧给——库文档
普遍未跑本地检查，检查事实现状恒缺席）；C 同意否决（entityRef 在场近似
＝「未验证伪装已验证」，W15 A7 同款）；**短期维持现状**（文档事实清单＋
「期望态非已验证」明示＝已是诚实终态，A 的增量仅是把文案明示提升为词表
态）；B 是检查事实真正产生后的正解（投影无事实可投时无增量），随检查
流程切片立项。最终映射规则需核心共同确认（权威侧原则解释权在核心）。
bdl-queries v0.4 读面切片已入 main；端到端链两翼已过验（389912e＋桌面
消费）；候补①②销账/消解。数据侧无在手切片，待命。
