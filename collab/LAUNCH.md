...
- **工作时段（本地 23:00–次日 08:40）内每 20 分钟**，把 `collab/TICK.md`「命令正文」代码块
  **原样**发给全部六个进程（六份文本完全相同）；错峰建议见 TICK 文件头；
- 非工作时段不发；进程自查 `date`，09:30 后强制截断（wip 提交，下夜继续）；
- 时间纪律写在命令正文里，发送方不用自己判断。
- **派发前在途核查（硬性，v1.3）**：每节拍派发某角色前，先核查该角色**零在途实例**
  （上例回执未到达＝在途；以会话实例登记目录 output.txt 缺席＋TaskOutput 探测为准）。
  在途即跳过该角色本轮派发——绝不凭记忆整组盲发。同角色双例会共享同一工作树互相污染，
  且并发总量过高会触发账户限流（1302）把在途例中跑击落（2026-09-17 00:3x 事故，
  四例中跑被击落、wt-3 留下半途混写，登记见 BOARD #34）。
...
- v1.3（2026-09-17）：新增派发前同角色在途硬核查规则（零在途才派发；双例同树污染＋限流
  击落在途例的事故见 BOARD #34）。
- v1.2（2026-09-15）：工作时段截止 08:30 延后至 08:40（用户指令，操作者 directed 批转达），
  与 TICK v1.5 同步。
