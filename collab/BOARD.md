...
最近更新：2026-09-07 05:4x（**W7 第二轮复验落账**：B3/B4 ✓ 场景条 ✓；i18n 动态载荷残留
与仓储 UI 残留文本未过——缺陷批未清，桌面第三轮修复中；**U7/U8 升级用户裁决**〔仓储
布局归属、实验性设置深度/协议升版时点〕；I-1 已关闭 16/16）
...
| M3 | 收口中 | 门项全完成：I-1 已交付（16/16）；余缺陷批用户复验 + 关门程序 a/b/c |
...
- [ ] W7 剩余 — **第二轮复验（用户按新实验入口执行）**：B3/B4 通知实体错误 ✓ 已修复；
  DEV 场景条收起 ✓；**i18n 仍未通过**（动态载荷中文残留：任务/通知标题与错误详情未走
  i18n，EN/JA 复现，截图证据）；**仓储 UI 残留文本**仍不通过。**M3 阻塞缺陷批未清**，
  桌面第三轮修复中（含通知移除 DOM 残影、✕ 图标语义重复等截图新发现）；两项范围仲裁
  升级用户裁决（见 U7/U8）。集成首轮仲裁「语义重构入 M4」的仓储布局一项被复验触及，
  以 U7 用户裁决为准
...
| U7 | 仓储布局（自适应列数+右侧详情区）的 M3/M4 归属 | 集成仲裁升级 | 复验将其列为「未解决」，与集成「入 M4」仲裁冲突——**(a) 维持 M4**（M3 发行说明如实声明已接受裁决待实施）或 **(b) 提前到 M3**（桌面第三轮一并做，关门顺延） |
| U8 | 实验性设置深度与协议升版时点 | 集成仲裁升级 | 用户反馈当前「只是一个开关」与原意（两级选项在设置-实验性页内）可能不符——**(a) 当前开关形态 M3 先行，完整形态（两级选项+持久化）M4**；或 **(b) 现在做 bdl-commands v0.2 升版**（数据已列硬前置：Schema+向量+消费测试+双语协议，**数据明示不宜在 M3 收口窗口做**，会顺延关门） |
