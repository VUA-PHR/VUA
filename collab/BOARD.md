...
| --- | --- | --- | --- | --- |
| O-1 | **W15 关门确认文档审阅**（桌面交付中，路径随交付回复转达） | W15 两修正项已回流验收（触发时机＝素材导入时语义＋「生成 VPM 包替代」术语）；M5 关门按门序剩 W15 用户确认此最后一项。桌面正在交付关门确认文档（F6 接线前呈现的诚实性验收：操作步骤＋成功/失败标准） | 按桌面交付的文档逐项走查设置-实验性页确认链（文档路径随交付回复转达后补入本行） | 成功＝用户确认两修正项（触发时机呈现与「生成 VPM 包替代」术语）→ M5 关门进入 W25 窗口流程；失败＝发现问题逐项记录回流桌面修复后重新走查 |
