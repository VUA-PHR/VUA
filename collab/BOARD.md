...
| 17 | proposal **014 已落地后的语义修订请求**（用户回传 W15 条目 6，2026-09-09）：U3/1.2.0「原项目只读＋唯一写路径＝新路径副本」重审为**两模式**——模式 1「迁移」：选择 VCC/ALCOM 项目，完全复制其包清单，项目变为 VUA 原生、可正常写操作；模式 2：不迁移/迁移未完成＝只读（与现行一致）。**[需用户] 三项澄清**：①模式 1 是原地接管还是仍复制到新目录？②若原地接管：1.2.0 rationale（跨工具锁不可行）如何缓解——迁移前置要求先在 VCC/ALCOM 移除登记？冲突风险用户自担？③模式 1 写操作范围＝全部写还是逐步开放？ | 用户（环境登记） | **待用户裁决**——环境影响：兼容矩阵/013/014 词表语义、桌面 T-C/F6 确认链；模式 2 与现行 014 副本导入兼容 |
| 18 | **Unity 项目备注功能**（用户回传 W15 条目 4，2026-09-09）：Unity（至少 2022）项目名无法兼容亚洲字符——提议 VUA 项目「备注」功能使含亚洲字符意图的项目在 VUA 内可识别。**[需用户] 两项确认**：①备注存储位置＝VUA 侧元数据（`.vua/` 或存储层，不写 Unity/原项目文件，符合 1.2.0）还是替代写入 Unity `ProjectSettings.productName`？②应用范围＝仅项目列表显示，还是兼作新项目命名指引（亚洲字符提示 Unity 限制）？处置草案：VUA 侧元数据备注＋列表备注列/徽标＋创建时命名提示 | 用户（环境登记） | **待用户裁决**——范围确认后环境出提案（环境元数据面＋桌面呈现） |
