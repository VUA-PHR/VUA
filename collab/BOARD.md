...
最近更新：2026-09-07 06:4x（**U7/U8 用户裁决销账**：仓储布局维持 M4；实验性设置开关
先行 M3、完整形态随 M4 bdl-commands v0.2。**M3 最终收口清单确定**：缺陷批第三轮修复→
用户复验→关门程序 a/b/c→v0.5.0+推送→M4 分配）
...
列数+右侧详情区，U7）；实验性设置完整形态（两级选项：生成 VPM 替代+生成后删除原始；
含持久化位置决策与 bdl-commands v0.2 升版硬前置，U8/005 备案）。
