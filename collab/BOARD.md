...
| U10 | **Unity 编辑器路径配置面立项**（激活 M3 素材直产链的壳侧缺口；桌面 workshop F3 迁移评估收口 2026-09-12 升级） | 桌面 | **待用户裁决**。事实：核心 provider 侧 `production.*` 七方法（v0.1，M3 冻结验收面）完整在且健康；恒不可用根因＝壳未注入 `VUA_UNITY_EDITOR`（该路径属用户机事实，不能硬编码）。激活＝新增「Unity 编辑器路径」小配置面（设置页手选＋environment 检测 unity_editors 项预填候选）。选项：**A＝立项小配置面**（M3 直产链激活，generateVpm 可用）；**B＝维持诚实不可用现状**（排期 M7+ 再议）；**C＝仅自动探测、不提供手动配置**（范围最小，探测失败即不可用）。桌面裁决前维持现状不猜路径 |
