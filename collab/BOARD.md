...
| 27 | **「未接入／手动填链接」干净启动复现监测（桌面义务，2026-09-15 晚登记，操作者 directed 批如实转达）**：用户实测反馈三症状（环境检测/仓储/包管理器显示未接入＋云端下载需手动填写链接）经操作者诊断＝残留 vite（PID 75496，2026-09-13 19:00 起）持有端口 5173 致启动链中断、Provider 未运行下的**诚实降级呈现**（非新缺陷证据：main 1175ecf ImportPage.tsx 实证含内嵌浏览实现 browseAvailability/EmbeddedBrowseState/viewId 且按 IMP-2 设计在 `desktop.remoteBrowser` 能力不可用时回退手动填链接；环境与路径页 U10 切片 6660d72、仓储布局、项目页 T-C 均在库）；用户已获重启指引（主库 `pnpm dev`） | 桌面（义务行；登记方＝集成） | **今晚工作窗口办理**：用户重启后若「未接入/手动填链接」任一症状在干净启动下复现→按候选缺陷登记并桌面定位修复（照 #25/#26 先例：用户实测报告→复现定位→修复）；不复现→本行销账。**桌面进程面复验注记（2026-09-15 23:4x 桌面 wt-3 今晚窗口办理）**：用户 23:08 常规重启后桌面进程面独立复验干净启动链完整（vite 64912 监听 5173＋Electron 四进程树均 23:08:10＋provider 54764 父进程＝electron main＝provider-bootstrap 受监督拉起链实证，全部属新树零残留）——**端口占用报错与「Provider 未运行」两前置均消除**；UI 呈现复验候用户回填后按结果销账或开候选缺陷（桌面不代用户宣称三症状消失）。**附带候选缺陷升级**：残留产生机制定位在 apps/desktop/scripts/dev.mjs（Windows shell:true 下 vite/electron 为孙进程，finally 块 kill() 仅杀直接子进程，非常规退出孙进程孤儿化＝端口占用复发风险），候选缺陷候集成归因排期（照 #22 先例），桌面候裁决开工不抢跑 |
