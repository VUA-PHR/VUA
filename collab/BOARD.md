...
| --- | --- | --- | --- | --- |
| O-1 | **W15 关门确认文档审阅**（**已交付 2026-09-09 06:16，路径=`VUA-3 工作树 docs/plans/w15-close-user-confirmation_ZH.md`**〔docs/plans 为本地草稿区，文档在 VUA-3 检出内〕） | W15 两修正项已回流验收（触发时机＝素材导入时语义＋「生成 VPM 包替代」术语）；M5 关门按门序剩 W15 用户确认此最后一项。桌面正在交付关门确认文档（F6 接线前呈现的诚实性验收：操作步骤＋成功/失败标准） | 按桌面交付的文档逐项走查设置-实验性页确认链（文档路径随交付回复转达后补入本行） | 成功＝用户确认两修正项（触发时机呈现与「生成 VPM 包替代」术语）→ M5 关门进入 W25 窗口流程；失败＝发现问题逐项记录回流桌面修复后重新走查 |
...
| # | 事项 | 提出方 | 状态/用户决定 |
| --- | --- | --- | --- |
| U-9 | **W15 复验 9 条新发现——意图确认与裁决**（桌面登记 2026-09-09；逐条理解/歧义/处置草案见 wt-3 状态文件留言区） | 桌面 | 需用户逐条裁决：①条 1 行 2 自动取消是否清持久偏好+已发生删除不溯（桌面理解=均是）②条 2 条目级模式编辑门控还是撤下+删除触发迁至导入编排（008/014 语义修订，跨域）③条 3 Web 浏览功能定义（BOOTH 目录浏览 vs 内嵌购买流）+审阅子智能体安排④条 7 「迁移」术语与「导入为 VUA 管理的副本」统一⑤条 9 per-Provider 开发模式架构方向与 DEV 边界——其中②③④涉及跨域/规格，①⑤桌面可即做（确认后） |
| U-10 | **W24 工作台三视图映射规格**（桌面请求 2026-09-09） | 桌面 | 需产品/用户给出三视图（图谱/列表/爆炸）与 recipe v0.3 文档的领域映射规格——规格到齐前 W24 读面消费切片不开工（读面 wire+TS 面已就绪） |
