...
最近更新：2026-09-08 04:3x（**用户批量裁决落账：U1 批准 006〔M6 获授权〕＋U3 裁决
落 product-boundary 1.2.0**——ALCOM/VCC 只读兼容＋「导入为 VUA 管理的副本」，
双语＋REGISTRY＋outline 2.0.7 F6 锚点；**W15 复验＝通过·附两修正项交付中**〔触发
时机=素材导入时＋术语「生成 VPM 包替代」，桌面本轮处理〕；M4 关门＝等修正项回流
＋用户确认；U5 维持暂缓）
...
| M6 | 未开窗·**已获产品语义授权** | 2026-09-08——U1 批准 proposal 006（EAC 恢复边界，R1–R9 无修订＋严化验收项）＋U3 裁决 F6 只读兼容语义（product-boundary 1.2.0）；开窗仍以 M4/M5 关门为序 |
...
**W15 第二轮复验（2026-09-08，用户）：通过**——交互逻辑认可（全局开关形态＋危险
确认弹窗＋术语方向）。附**两修正项（交付中，桌面本轮处理）**：
1. **触发时机**：生成 VPM 包替代应为**素材导入时**（现文案与实际逻辑均为「装配时」
   ——含实际逻辑修正；若跨域到 provider/装配管线由操作者另行路由）；
2. **术语**：「生成 VPM 替代」→「生成 VPM 包替代」（应用内 i18n 文案；受管文档面
   已在 product-boundary 1.1.1/1.2.0 与 outline 2.0.6/2.0.7 合规）。
**M4 关门条件＝两修正项回流＋用户确认**（复验本身已通过，不再整轮重走）。**W17 已入表**（outline 2.0.4）且 **✅ 全链交付**：桌面协作面（869519b 自并 c93ac5e：
...
| # | 事项 | 提出方 | 状态/用户决定 |
| --- | --- | --- | --- |
| U1 | EAC 实验性恢复的边界裁决稿（proposal 006；集成仲裁已过：R1–R9 无修订） | 环境 | **✅ 用户已批准（2026-09-08 批量裁决，第三方仲裁复核后）**：006 R1–R9 无修订整体批准；附更严验收标准——「终止能力在允许清单为空时显示未核验/不可用」（实现期验收，环境落账 006 状态改已接受并写入八点批准语义）；**M6 相关实现获授权** |
