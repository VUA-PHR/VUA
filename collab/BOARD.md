...
最近更新：2026-09-08 04:1x（**proposal 008 已接受·路径 a**〔两域表态一致，集成
仲裁落提案线程；接线切片归 M5 首批；v0.2 维持冻结〕；design-standard **0.6.3**
桌面自并验收〔两态开关模型＋§8.3 随 W15 裁决改版〕；amf-unity 缩进清理合并；
M4 门验收＝第二轮用户走查不变）
...
| # | 事项 | 提出方 | 状态/用户决定 |
| --- | --- | --- | --- |
| U1 | EAC 实验性恢复的边界裁决稿（proposal 006；集成仲裁已过：R1–R9 无修订） | 环境 | **✅ 已销账（2026-09-08）：用户整体批准 R1–R9，无修订**（批准语义八点全文见 006 内联线程；R8 实现措辞按用户第 2 点收紧为「未核验/不可用」）。M6 实现已获准，开窗后环境按 R9 执行 |
