...
// storage-keys.ts(键唯一来源纪律),其字符串随主包合法存在——故不纳入
// 指纹集(纳入即常量性误报)。本断言覆盖范围:仅 fixture 负载字符串;
// 不覆盖 DEV 分支代码的构建期剔除是否生效(该键被读取的调用点在
// import.meta.env.DEV 分支内,剔除由静态替换＋Rollup 死代码消除机制
// 保证;本脚本无对应检查,如需断言须另行专项检查)。
