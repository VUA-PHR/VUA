...
/** BOOTH 登录页(W25 走查缺陷③b):未登录线索("none")时首开引导至此而非
 ...
 * 即解除武装 = 清理在飞计时——
 * - 失败到达:不武装(失败驻留,不静默关走);
 * - 用户接管(受理窗口内再次发起拾取/采纳,反馈被清空):取消在飞计时
 *   ——自动关闭不跑在用户进行中的操作下面(否则原生拾取对话框停留期间
 *   弹窗静默关闭,拾取结果落在已卸载组件上被丢弃,操作上下文丢失);
 * - 同窗二次受理:必经「反馈清空 → 再置受理」两拍,引用变化重新武装 =
 *   从最后一次受理重新计时(同第 178 批登记语义)。 */
export function autoCloseArmed(
  feedback: { readonly kind: "accepted" | "failure" | "notice" } | null,
): boolean {
  return feedback?.kind === "accepted";
}
