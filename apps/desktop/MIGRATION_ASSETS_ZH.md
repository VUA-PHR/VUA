...
KIMI 表现层其余资产（功能页面、应用模型、4 语言 i18n、WebGL 场景、原语补齐）与 5 个质量门脚本迁入 Electron 壳。开发排期见 `docs/development-outline_ZH.md`（F2–F7 各门的应用契约接入仍按计划推进；本轮只恢复表现层与纯模型，不伪造生产数据）。
