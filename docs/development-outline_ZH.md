...
| 任务 | 负责角色 | 协作 | 状态 |
| --- | --- | --- | --- |
| 远程内容/Session/下载端口与隔离浏览界面 | 桌面 | — | ✅ 已交付（F4-2/3/4/6），走查闭环（W6/W7）。**诚实降级补注（2026-09-09 审阅 M-1）**：应用内浏览窗口呈现未迁移（当前诚实降级为系统浏览器），接线归素材导入页阶段（M6 IMP-2，待用户裁决 2 落地）——不改变本行验收结论 |
