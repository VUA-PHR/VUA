...
> 快照日期：2026-09-06（2026-09-07 增补 W12 并更新进展：W2–W9 已交付回流 main；W1/I-1
> 完成 16/16 真机格并合并；W10 已随 M3 验收冻结；W11 关门执行中；W12 待 M4 开窗）。
...
| W12 | catalog 观察管线服务面（`catalog.list/detail/status` 的 Rust 组装，词表已随 bdl-queries v0.3 冻结） | 数据 | 桌面 | `bdl_queries.rs` 预留头注；**M4 已开窗，数据首切片即领** |
| W13 | 仓储布局重构（自适应列数+右侧详情区） | 桌面 | — | U7 裁决 M4 实施；验收=设计标准同步+走查 |
| W14 | bdl-commands v0.2 升版（两级选项语义+持久化位置决策） | 数据 | 桌面 | U8/005 备案硬前置：Schema+向量+消费测试+双语协议 |
| W15 | 设置-实验性完整形态（两级选项入口） | 桌面 | 数据 | 依赖 W14；验收=用户走查 |
| W16 | 设计标准同步（通知中心/仓储布局/实验性语义） | 桌面 | — | 007 裁决与 U7 落地后的设计标准修订 |
...
| catalog 观察管线服务面 | 数据 | 桌面 | W12（M4 首切片） |
| 仓储布局重构（自适应列数+右侧详情区） | 桌面 | — | W13（U7 裁决 M4 实施） |
| bdl-commands v0.2 升版（两级选项语义+持久化位置决策） | 数据 | 桌面 | W14（U8/005 备案硬前置） |
| 设置-实验性完整形态（两级选项入口） | 桌面 | 数据 | W15（依赖 W14） |
| 设计标准同步（通知中心/仓储布局/实验性语义） | 桌面 | — | W16 |
| 门验收与发行 | 集成 | 全部 | M3 已关闭（v0.5.0），M4 验收按门序 |
