...
| W12 | catalog 观察管线服务面（`catalog.list/detail/status` 的 Rust 组装，词表已随 bdl-queries v0.3 冻结） | 数据 | 桌面 | `bdl_queries.rs` 预留头注；M4 开窗执行（此前 W6/W7 走查以桌面 fixture/观察面为准，不构成数据侧在途等待） |
...
- 2.0.2（2026-09-07）：M3 收口更新——I-1 完成（16/16 真机格）、当前窗口进展注记、
  W10/production-use-case v0.1 与 amf-production v0.2 随 M3 验收冻结。
