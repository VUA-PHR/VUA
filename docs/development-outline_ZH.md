...
> 快照日期：2026-09-08（2.0.5 更新：W17 全链交付〔桌面呈现接线 c93ac5e＋数据写入面
> 73cae1b〕；W12 全链闭环；M4 门验收只剩 W15 用户走查）。
...
| W17 | 观察管线写入侧（观察数据写入 BDL products 表；伴随 errors.catalog.* 在 catalog 视图的透传呈现接线） | 数据 | 桌面 | **✅ 全链交付**：数据写入面 eb899f1（合并 73cae1b：products 全列 upsert＋catalog_updated_seq 簿记＋读组装消费观察列＋9 项消费测试）＋桌面呈现接线 869519b（合并 c93ac5e） |
...
### B3 前置 Spike（无产品版本）：VPM 包制作与安装路径
...
- **文档**：记录首个端到端用例、错误/恢复语义、最小 Build Record、Bridge 操作覆盖，以及直接
  导入与本地 VPM 包制作/安装的两条素材入口契约；
- **迁移**：只提取该切片所需的旧页面行为和 Unity 语义，完成对应迁移台账；
- **整合**：合并 F3 与 B3，以合成 Avatar、一件合成衣装和全球版 Unity `2022.3.22f1`，分别通过
  `.unitypackage` 直接导入和本地 VPM 包制作后由 VUA 包管理器安装两条路径跑通 Inspect 至 Recover；
- **交付**：两条路径均通过成功、取消、漂移、超时、Bridge 拒绝、回滚成功/失败和幂等重放；VPM 包
...
| 观察管线写入侧（products 表写入）＋呈现接线 | 数据 | 桌面 | ✅ 全链交付（W17：写入面 73cae1b＋呈现接线 c93ac5e） |
...
| F6 引导/计划/确认页面与逐次警告确认（含 U3 只读兼容呈现：写操作交接与「导入为 VUA 管理的副本」入口，见 product-boundary 1.2.0） | 桌面 | 环境 |
| 门验收与发行 | 集成 | 全部 |
...
- **F6 项目与环境页面**：见 M6 任务分解表；含 U3 裁决的 ALCOM/VCC 只读兼容呈现
  （写操作交接、显式「导入为 VUA 管理的副本」入口，product-boundary 1.2.0）。
...
- 2.0.7（2026-09-08）：U3 用户裁决锚点落表——M6 分解表 F6 行与 F6 引用行补注
  ALCOM/VCC 只读兼容呈现语义（写操作交接与显式「导入为 VUA 管理的副本」入口，
  权威语义见 product-boundary 1.2.0；任务行细化在 M6 开窗时随分解表进行）；双语同步。
- 2.0.6（2026-09-08）：术语修正（用户硬裁定，2026-09-08）——VPM = VRChat Package
  Manager（管理器），VPM 包 = VPM package（被管理的包）；B3 前置 Spike 标题与 M3
  文档/整合/交付条目中「VPM 素材包」「本地 VPM 制作」「VPM 结果」统一为「VPM 包」
  语形；`generate-VPM` 等操作/提案标识符按专有名保留；双语同步。
- 2.0.5（2026-09-08）：W17 全链交付落表——数据写入面（products 全列 upsert＋
  catalog_updated_seq 簿记＋catalog 读组装消费观察列，73cae1b）＋桌面呈现接线
  （错误透传白名单＋live-acquire 旧码修复，c93ac5e）；W11 补充徽章全绿注记；
  双语同步。
