...
M 门 = **发布闸门＋版本地图**（2.1.0 重定义，提案 031-E6）：集成分支上的 tag + 验收清单，
门清单全绿后打 tag、发布发行说明。M 门按顺序关闭；前一 M 门确认的契约和迁移结果是下一
阶段的共同基线；不依赖未决契约的工作可以提前在切片中推进，但不能绕过对应 M 门形成产品
发行。**M 门不再承担工作清单**：门一开即冻结该门范围，之后新工作一律以新 proposal 立项
（`collab/proposals/`）并路由到下一门；事实工作队列见 `collab/BOARD.md` 开放问题表与
proposals，不在门表内增补。
...
## 当前窗口（M5：Recipe 与 AMF 生产主线，v0.7.0）

> 快照日期：2026-09-08（2.0.8 更新：**M4 已关门（v0.6.0，2026-09-08）**——W12–W17
> 全链交付、W15 复验通过＋两修正项回流＋用户确认；M5 开窗，M4 分解表全部落定见
> M4 节）。本窗口的全部任务已分解到角色；完成后由集成角色验收并推进 M5 门。
>
> 窗口状态（2.0.13 更新，2026-09-20）：**W25 真机窗口已由用户指令开启（2026-09-19/20，
> 时段例外照用户指令执行）**——各角色 W25 候办照 O-2 执行序与各自候办清单在窗内办理；
> 集成席位已先行办理 W26 步骤 a（文档审查）的前置盘点＝面向用户/操作者的受管文档审计
> （清单落 `collab/proposals/028-user-facing-doc-audit.md`，先出清单再逐域更新）。
> 零端到端宣称纪律不变：窗内一切产出以真机证据为准逐项登记，未跑的段如实申报未跑。
>
> 门状态（2.1.0 显式标注，提案 031-E6/E7）：**M5 功能收口完成（实现面在库），唯一候项
> ＝W25 真机验收**。W25 系「等用户」项（需用户亲自启动 VRChat），本文档不设绕行机制；
> 门验收与发行（W26）候 W25 真机证据在案后按 M4 先例执行。

| # | 任务 | 负责角色 | 协作 | 锚点/验收 |
| --- | --- | --- | --- | --- |
| W18 | **M5 首批**：proposal 008 路径 a 接线——「生成后删除原始素材文件」开关＝桌面本地偏好，生成 Done 回执后逐条调用既有 `warehouse.deleteOriginals`（每条独立审计任务） | 桌面 | 核心、数据 | proposal 008 已接受（集成仲裁 2026-09-08）；权威边界＝桌面只握发起时机，执行/守卫/审计留 provider 任务面 |
| W19 | **M5 首批（合并设计）**：素材导入时自动生成挂点与编排语义（生成任务在导入流内的发起语义；桌面呈现＋核心/数据任务面语义同批设计） | 桌面 | 核心、数据 | product-boundary 1.2.1 触发时机语义；诚实标注随接线移除 |
| W20 | Recipe v0.3、Local Resolution、版本锁 | 核心 | 数据 | M5 分解表既有行；2.0.14 注：Recipe 字段白名单与编码、对象定位、冲突处理实现为用户裁决 2026-09-22 的未决/待实测项，不构成排期承诺 |
| W21 | Unity Bridge 操作扩展（dry-run、幂等、恢复）与 C# 侧实现 | 产线 | 核心 | M5 分解表既有行 |
| W22 | 完整 Build Record（计划差异、证据摘要） | 核心 | 产线 | M5 分解表既有行 |
| W23 | 兼容/缺失证据模型 | 数据 | 核心 | M5 分解表既有行；2.0.14 注：自动兼容性取证＝默认关闭的实验功能，不作核心流程既定前置；关闭取证不影响 BDL 基础存储/普通导入/来源补充；没证据＝未知 |
| W24 | Recipe/Assembly 工作台（三视图共享选择与领域语义） | 桌面 | 核心 | M5 分解表既有行；2.0.14 注：验收口径改按用户裁决 2026-09-22 新方向——Wizard 选路径、Recipe 冲突四选项、素材来源分享时再补（已接受方向，尚未实现） |
| W25 | 合法自有素材冒烟路径与复现 | 产线 | 集成 | M5 分解表既有行；真机窗口与合法素材环境变量；**窗口已由用户指令开启（2026-09-19/20，时段例外）** |
| W26 | 门验收与发行（v0.7.0） | 集成 | 全部 | 按 M4 关门先例：文档审查→门项核实→版本/发行/推送；步骤 a 前置盘点＝提案 028 面向用户/操作者文档审计（2026-09-20 清单已落） |

> 2.0.14 注（用户裁决 2026-09-22，权威正文见 product-boundary 1.5.0）：对象定位、Recipe 字段
> 白名单与编码、冲突处理实现、依赖降级路径、BDL 人工修订五项为未决/待实测事项，本计划
> 任何任务行均不得将其读作排期承诺；W20/W23/W24 行验收口径随批调整（原口径保留备查、
> 明确标注被取代）。
>
> 2.1.0 注（提案 031-E1/E6）：**本 W 窗口表定格为历史记录，W 号停止增发**；新工作一律以
> 新 proposal 立项（`collab/proposals/`），事实工作队列见 `collab/BOARD.md` 开放问题表
> 与 proposals。表内既有行不追溯改写。
...
任务分解（2026-09-07 M4 分配核对：历史进度逐项核实后更新；2026-09-08 关门落定）：

| 任务 | 负责角色 | 协作 | 状态 |
| --- | --- | --- | --- |
| 远程内容/Session/下载端口与隔离浏览界面 | 桌面 | — | ✅ 已交付（F4-2/3/4/6），走查闭环（W6/W7）。**诚实降级补注（2026-09-09 审阅 M-1）**：应用内浏览窗口呈现未迁移（当前诚实降级为系统浏览器），接线归素材导入页阶段（**M6 IMP-2**——增设已裁决〔U7① 2026-09-09〕、行已入 M6 分解表〔2.0.11〕；导航/新窗口/外部协议语义权威＝用户「新窗口/外部协议」四分法细则〔2026-09-09，BOARD U9〕）——不改变本行验收结论 |
...
| 仓储布局重构（自适应列数+右侧详情区） | 桌面 | — | ✅ 已合并（W13，88b4551）；随 M4 关门用户整体确认（2026-09-08） |
| bdl-commands v0.2 升版（两级选项语义+持久化位置决策） | 数据 | 桌面 | ✅ 已交付冻结（W14，bdl-commands v0.2）；provider 路由登记完成（核心 10325cd） |
| 设置-实验性完整形态（两级选项入口） | 桌面 | 数据 | ✅ **闭环（W15）**：首轮走查不通过→按用户示意图重做（全局开关形态，4fb6411）＋修正批（触发时机=素材导入时语义＋「VPM 包替代」术语＋未接线诚实标注，2fa260d）→第二轮复验通过＋用户确认关门（2026-09-08） |
| 设计标准同步（通知中心/仓储布局/实验性语义） | 桌面 | — | ✅ 已交付（W16，设计标准 v0.6.2；0.6.3 随 W15 重做改版） |
| 门验收与发行 | 集成 | 全部 | ✅ **M4 已关门（v0.6.0，2026-09-08）**：W12–W17 全链证据＋CI/全量绿＋008 仲裁路径 a＋复验通过＋用户确认 |
...
任务分解（2026-09-08 M5 开窗：历史进度核对〔无预交付〕后落当前窗口 W18–W26；桌面
M5 首批为 proposal 008 接线与导入时挂点合并设计）：

| 任务 | 负责角色 | 协作 | 当前窗口行 |
| --- | --- | --- | --- |
| Recipe v0.3、Local Resolution、版本锁 | 核心 | 数据 | W20 |
| Unity Bridge 操作扩展（dry-run、幂等、恢复）与 C# 侧实现 | 产线 | 核心 | W21 |
| 完整 Build Record（计划差异、证据摘要） | 核心 | 产线 | W22 |
| 兼容/缺失证据模型 | 数据 | 核心 | W23 |
| Recipe/Assembly 工作台（三视图共享选择与领域语义） | 桌面 | 核心 | W24 |
| 合法自有素材冒烟路径与复现 | 产线 | 集成 | W25 |
| 008 路径 a 接线（生成后删除原始素材）＋导入时自动生成挂点与编排语义（合并设计，M5 首批） | 桌面 | 核心、数据 | W18/W19 |
| 门验收与发行 | 集成 | 全部 | W26 |

> 2.1.0 门状态（提案 031-E6/E7，显式门状态不设绕行机制）：**M5 功能收口完成（实现面在
> 库），唯一候项＝W25 真机验收**——开窗期间新增工作（019/026/029/030 等）已由 proposals
> 承载、不在本门清单内增补；本门自 2.1.0 起为发布闸门＋版本地图行，关门候 W25 真机证据
> 在案后按 W26 程序（M4 先例）执行。
...
任务分解（**项目管理部分提前开工**——用户裁决 2026-09-08 晚，操作者转达；环境/桌面
已同步派发，**越过门序**但门验收与发行仍等 M5 关门后按门序。治理注记：T-A 项目
管理命令 wire 词表为**新协议面**——桌面提案→核心裁决流程不变；early-open 授权
记录见 BOARD M6 行。**IMP-1~5 素材导入页任务包增设**（用户裁决 2026-09-09：
U7① 同意增设、细节允许先粗后细；U7③ 下载落库协议面数据主导冻结硬前置；
IMP-2 导航/新窗口/外部协议语义权威＝用户「新窗口/外部协议」四分法实施细则
〔2026-09-09，「同意按此方案执行」，BOARD U9〕；IMP 门验收**并入** M6〔v0.8.0〕
门验收清单——默认读法登记，用户未表异议；实现 2026-09-09 23:00 开工，桌面牵头））：

| 任务包 | 任务 | 负责角色 | 协作 | 锚点/备注 |
| --- | --- | --- | --- | --- |
| T-A | 通用 `vrc-get` 项目与包管理路径（含项目管理命令 wire 词表——新协议面，桌面提案→核心裁决） | 环境 | 核心 | 治理注记见上；wire 词表提案由桌面起草、核心裁决 |
| T-B | ALCOM/VCC 能力检测与兼容矩阵 | 环境 | 桌面 | U3 只读兼容语义，权威＝product-boundary 1.2.0 |
| T-C | F6 引导/计划/确认页面与逐次警告确认（含 U3 只读兼容呈现：写操作交接与「导入为 VUA 管理的副本」入口） | 桌面 | 环境 | 权威＝product-boundary 1.2.0；W24 读面前置随 M5 production-use-case v0.2 |
| EAC | EAC 实验性恢复适配器（R1–R9 已获用户批准，proposal 006 已接受；按 R9 执行序开实现切片） | 环境 | 集成（裁决） | 八点批准语义见 006 内联线程 |
| 环境 | Unity/VRChat/SteamVR 环境检查、网络/磁盘/残留进程失败处理 | 环境 | 核心 | 含在提前授权范围 |
| IMP-1 | 素材导入页信息架构与呈现设计：独立导入面（云端下载＋本地导入两部分）；与设计标准 §8.3「连续素材获取路径」对账（双轨头去留、云端目录轨合并方式）；隔离 Session 登录呈现与「购买流不做」语义呈现 | 桌面 | 集成（设计标准对账与升版）、数据、核心 | 裁决前置已齐（U7①＋U9）；与 W15 收尾修正批同动仓储/设置页，排期归集成 |
| IMP-2 | 应用内 Web 浏览接线：渲染层消费既有 `RemoteContentApiV1`＋Main 基座（remote-content/download-port 在位）；`desktop.remoteBrowser` 能力判定翻转＋未接线诚实降级保留；导航策略按「清单内直行/清单外提示放行」翻转；**新窗口/外部协议/伪协议/手势按 U9 四分法细则实现**（`setWindowOpenHandler` 对原生新窗口一律 deny）；初始浏览清单提案（入库文本只写域名）；下载主机域＝真机验证→逐域提案→批准 | 桌面 | 核心（contracts/capability 面）、集成（安全验收与 product-boundary/desktop 架构落账） | 硬前置＝U9 细则落受管文档后动工；隔离红线（sandbox/无 preload/独立 partition/标准 Web API）逐条不动；未验证下载域一律拒绝（诚实失败） |
| IMP-3 | 云端下载→仓储条目落库路径（`downloaded_material`）——**契约先行**：数据域主导 bdl-commands 族升版（Schema＋正负例向量＋消费测试＋桌面 TS 登记＋双语协议＋REGISTRY）冻结硬前置后实现 | 数据 | 核心（wire 路由）、桌面（TS 面与呈现） | 硬前置＝U7③ 程序；禁止平行第三条路（只准 bdl-commands 族或其升版）；契约设计可与 IMP-1 并行启动，不占门序 |
| IMP-4 | 本地导入呈现收口并入导入页：复用 `warehouse.import`（bdl-commands v0.3）与既有 W18 提交流（拾取→确认列表→单命令→任务中心）——既有导入 UI 的入口收敛/重组，零新增词表零新增路径 | 桌面 | 数据 | 协议与实现已在 main（v0.3 冻结＋W18 呈现批）；可与 IMP-1 同批 |
| IMP-5 | 验收与文档同步：隔离冒烟（Session 隔离/权限拒绝/清单外浏览提示/清单外下载拒绝/下载接管/新窗口与外部协议行为按 U9 细则）、诚实空态与能力判定核验；product-boundary 浏览白名单策略落版／outline M6 表／design-standard／desktop 架构行更新 | 桌面（文档协作） | 集成（门验收与文档落账）、数据、核心 | 证据纪律：无真机证据不宣称端到端（BOOTH 下载域真机验证记录为 IMP-2 交付物）；IMP 完成度进入 v0.8.0 发行条件（并入读法） |
| — | 门验收与发行（**不在提前授权范围**） | 集成 | 全部 | 等 M5 关门后按门序 |
...
任务分解（**部分提前开工**——用户裁决 2026-09-15，操作者转达；模式照 M6 节先例：
前四行任务包〔检查证据／报告快照与只读服务／Inspection/Release 页面与官方 SDK
交接／桌面 Overlay 收尾〕**越过门序**可开工，**门验收与发行不在提前授权范围**、
仍候 M5 关门后按门序；既有契约前置已在库＝016 链四件〔inspection-evidence v0.1
冻结＋unity-bridge v3 冻结含三只读检查操作锚点实现 7d63abe＋核心检查切片实现批
7a262b8＋inspection-queries v0.1 冻结〕，early-open 授权记录见 BOARD M7 行；各
角色按锚点领取，开工前先合并 main 最新）：
...
| 门验收与发行（**不在提前授权范围**） | 集成 | 全部 |

**2.0.14 注（用户裁决 2026-09-22）**：本门「检查证据」与「Inspection/Release 页面与官方 SDK
交接」行的验收口径被取代——检测收进制作记录（Release 运行占位记录＋通知中心双通道）；
未执行的检查不得显示为通过；最终上传由用户在官方 SDK 完成，技术检查不保证外观行为符合
预期。上表行保留备查，新方向为已接受、尚未实现。
...
| v1.5 | Avatar 兼容性清洗建库（2.0.14 注：自动兼容性取证为默认关闭的实验功能，本方向不以它为既定前置，启动前须独立接受决策） |

运行时工具集成、社区插件执行与插件市场在正式版 `1.0.0` 前不实施（产品边界既定）。

## 文档变更日志

- 2.1.0（2026-09-23）：**M 门重定义为「发布闸门＋版本地图」**（提案 031-E6/E7，用户
  2026-09-23 裁决采纳方案 A〔发布/计划解耦＋工作项单命名空间〕，提案 031 拍 2 执行）——
  「目标与基线」节 M 门定义改为：M 门不再承担工作清单，门一开即冻结范围，之后新工作
  一律以新 proposal 立项并路由到下一门，事实工作队列见 BOARD 开放问题表与 proposals；
  当前窗口节与 M5 节落**显式门状态**＝M5 功能收口完成（实现面在库），唯一候项＝W25
  真机验收（W25 系「等用户」项，不设绕行机制）；W 窗口表定格为历史记录、W 号停止增发
  （E1 同源）。M 门状态表 M8–M10 补行落在 `collab/BOARD.md`（本文件 M8–M10 分解节
  既有、不动）。产品版本不动（0.6.0 候 M5），不代跑 W25，不推翻 M0–M4 关门成绩，
  历史记录零删除。EN 镜像同步。
- 2.0.14（2026-09-22）：**用户裁决 2026-09-22（product-boundary 1.5.0）排期口径修正**——
  当前窗口 W20/W23/W24 行与 M7 分解表按新裁决调整验收口径（原口径保留备查、明确标注被
  取代）：W23 自动兼容性取证＝默认关闭的实验功能，不作核心流程既定前置（关闭不影响 BDL
  基础存储/普通导入/来源补充；没证据＝未知）；对象定位、Recipe 字段白名单与编码、冲突处理
  实现、依赖降级路径、BDL 人工修订五项标注未决/待实测，不得读作排期承诺；M7 检测口径改为
  收进制作记录（运行占位记录＋通知中心双通道、未执行≠通过、最终上传由用户在官方 SDK
  完成）；「`1.0.0` 之后」v1.5 兼容性建库方向同步加注。EN 镜像同步。
- 2.0.13（2026-09-20）：**W25 真机窗口开窗事实登记**（用户指令 2026-09-19/20，时段例外
  照用户指令执行）——当前窗口节增补窗口状态段：各角色 W25 候办照 O-2 执行序在窗内办理，
  集成席位先行办理 W26 步骤 a 前置盘点＝面向用户/操作者的受管文档审计（清单落
  `collab/proposals/028-user-facing-doc-audit.md`）；W25/W26 行随注。零端到端宣称纪律
  不变。EN 镜像同步。
- 2.0.12（2026-09-15）：**M7（v0.9.0）任务分解表前四行提前开工授权注记**（用户裁决
  2026-09-15，操作者转达）——模式照 M6 节先例：分解表前四行〔检查证据／报告快照与
  只读服务／Inspection/Release 页面与官方 SDK 交接／桌面 Overlay 收尾〕越过门序可
  开工；门验收与发行不在提前授权范围、候 M5 关门门序；既有契约前置已在库＝016 链
  四件（inspection-evidence v0.1 冻结＋unity-bridge v3 冻结含三只读检查操作锚点
  实现 7d63abe＋核心检查切片实现批 7a262b8＋inspection-queries v0.1 冻结）；各角色
  按锚点领取，开工前先合并 main 最新；授权记录见 BOARD M7 行；双语同步。
- 2.0.11（2026-09-09）：**M6 增设素材导入页任务包 IMP-1~5 落表**（用户裁决 2026-09-09
  批准增设〔U7①〕：IMP-1 设计／IMP-2 Web 浏览接线／IMP-3 云端下载落库契约先行
  〔数据〕／IMP-4 本地导入收口／IMP-5 验收文档；桌面牵头、数据/核心协作、集成
  验收与文档协作）；治理注记扩充——IMP-2 导航/新窗口/外部协议语义权威＝用户
  「新窗口/外部协议」四分法实施细则（2026-09-09，「同意按此方案执行」，BOARD
  U9 登记）；IMP 门验收并入 M6（v0.8.0）门验收清单（默认读法登记，用户未表异议）；
  M4 表「远程内容」行前向引用「M6 IMP-2」的挂起条件（待用户裁决 2）解除，引用
  随本批落账闭环；双语同步。
- 2.0.10（2026-09-09）：审阅报告 M-1 落账（诚实纪律）——M4 分解表「远程内容/Session/
  下载端口与隔离浏览界面」行内补注：应用内浏览窗口呈现未迁移（当前诚实降级为系统
  浏览器），接线归素材导入页阶段（M6 IMP-2，待用户裁决 2 落地）——不改变本行验收
  结论；双语同步。
- 2.0.9（2026-09-08 晚/09 凌晨落账）：**M6 项目管理部分提前开工落表**（用户裁决
  2026-09-08 晚，操作者转达）——M6 分解表按任务包拆行细化（T-A 通用 vrc-get 路径
  ／T-B ALCOM/VCC 能力检测／T-C F6 页面含 U3 锚点与 product-boundary 1.2.0 权威
  引用／EAC 适配器〔R1–R9 已批准〕／环境检查行）；治理注记：T-A wire 词表为新
  协议面（桌面提案→核心裁决流程不变）；门验收与发行不在提前授权范围（等 M5
  关门后按门序）；双语同步。
- 2.0.8（2026-09-08）：**M4 关门落表＋M5 开窗**——当前窗口重写为 M5（W18–W26：
  桌面 M5 首批 008 路径 a 接线＋导入时挂点合并设计；核心 Recipe v0.3/Local
  Resolution/版本锁/完整 Build Record；产线 Bridge 操作扩展/合法素材冒烟；数据
  兼容/缺失证据模型）；M4 分解表全部落定（W15 闭环：重做→复验通过→修正批→用户
  确认；W13 随关门用户整体确认；W14 路由登记完成注记）；触发时机语义（素材导入时）
  随复验修正落 product-boundary 1.2.1；双语同步。
