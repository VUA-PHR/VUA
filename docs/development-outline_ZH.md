...
> 快照日期：2026-09-07（2.0.3 更新：M3 已关门 v0.5.0；W11 执行完成；W12/W13/W14/W16
> 已交付合并，W15 已解锁；rust/ts CI 徽章红为两域内缺陷，见 BOARD #7/#8）。
...
| W12 | catalog 观察管线服务面（`catalog.list/detail/status` 的 Rust 组装，词表已随 bdl-queries v0.3 冻结） | 数据 | 桌面 | **✅ 已交付（6062a13 合并）**；收口待核心 provider-host 注册 catalog.* 路由 |
...
| catalog 观察管线服务面 | 数据 | 桌面 | ✅ 已交付（W12，6062a13）；收口待核心 catalog.* 路由 |
