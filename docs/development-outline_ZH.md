...
| W20 | Recipe v0.3、Local Resolution、版本锁 | 核心 | 数据 | M5 分解表既有行 |
| W21 | Unity Bridge 操作扩展（dry-run、幂等、恢复）与 C# 侧实现 | 产线 | 核心 | M5 分解表既有行 |
| W22 | 完整 Build Record（计划差异、证据摘要） | 核心 | 产线 | M5 分解表既有行 |
| W23 | 兼容/缺失证据模型 | 数据 | 核心 | M5 分解表既有行 |
| W24 | Recipe/Assembly 工作台（三视图共享选择与领域语义） | 桌面 | 核心 | M5 分解表既有行 |
