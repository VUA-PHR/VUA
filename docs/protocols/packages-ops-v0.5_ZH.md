...
> 目录；词面未接线：wire 路由与 served 行候核心下一接线切片，落地前
> 本方法在 wire 面不存在、桌面无写入口）**
...
> 10 负向量；核心消费测试
> `crates/provider-host/tests/packages_ops_consumer_v05.rs`；TS 守卫
...
> 候下一接线切片；双实现（库路径 `create_from_template`／CLI 路径
> `vpm new`）已在库＝环境域（实现核对切片照 024/025 程序）；桌面消
> 费＝桌面域（逐面升级，create 能力呈现须新立、不可复用 `blocks`
> ——语义＝变更预览可用性，与「可新建项目」不同构）
> 更新：2026-09-19（v0.5 冻结批：双 Schema＋向量＋核心消费测试＋TS
...
- **服务门（候接线切片落地）**：served 行 `packages.createOps` 一
  行服务一方法（registerOps/removeOps 一行先例）；行可用性＝后端
  `capabilities().create_project` 位；wire 路由 submit 前读位，假位
  答通用 `capability_missing`。信封常量照 A3/A4 先例候接线批于本文
  0.5.x 修订载明。
...
每条 wire 行自带版本常量）。v0.1 移除方法继续在 v0.1 词面应答、
...
- **词面已冻结、未接线。** wire 路由（`packages.createOps` served
  行与路由臂）、信封组装候核心下一接线切片——自本批起至接线落地，
  `packages.createProject` 在 wire 面不存在、桌面无任何创建入口、
  mock 恒答诚实缺席。双实现在库（库路径/CLI 路径），环境实现核对
  切片照 024/025 程序随接线批后办理；桌面消费切片候形状核可。wire
  测试骑真帧环与假后端归接线批；端到端走查归 W25（候用户开窗
  O-2）。本文档不宣称任何真机行为。
