...
> 更新：2026-09-23  
> 权威：`docs/product-boundary_ZH.md` 1.5.0（用户裁决 U3，2026-09-08；设置面例外 U14，2026-09-19）

## 权威与硬边界

本矩阵是产品边界 1.5.0 的展开，不引入新语义。VUA 对 ALCOM/VCC 管理的原项目**只读**；
唯一写路径是用户显式选择的「导入为 VUA 管理的副本」（新项目路径＋新项目身份，明示磁盘
占用，不复制可再生目录与旧任务状态，导入后重新 Inspect，保留来源关系）。`1.0.x` 边界内
对原项目的写能力一律 false；未来开放仅经新用户裁决。

**设置面例外（用户裁决 U14，2026-09-19）**：VPM 包管理设置（`settings.json` 的仓库订阅
与本地包注册表面）与 VCC/ALCOM 共享同一文件，VUA 经裁决读写该面且改动双方立即可见；
`vcc.liteDb` 等其余存储面维持禁止不变，项目文件面只读不变（权威措辞见产品边界 1.5.0
「明确边界」节）。
...
ALCOM/VCC 的注册表/数据库/设置/缓存（**例外**：`settings.json` 的 VPM 包管理设置面——
仓库订阅与本地包注册表面——经用户裁决 U14 共享读写，见上文「设置面例外」）；静默把原
项目改标为 VUA 管理。
...
- 1.3.1（2026-09-23）：权威指针刷新至产品边界 1.5.0——已核对 1.5.0「明确边界」节 U3/U14
  条款措辞与 1.4.0 一致，本矩阵语义零变更。EN 镜像同步。
- 1.3.0（2026-09-20）：U14 设置面例外落矩阵（提案 028 #5 路由，集成代笔修订）——「权威与
  硬边界」节补「设置面例外」段＋禁止清单补 `settings.json` 包管理设置面豁免注记（对齐产品
  边界 1.4.0 措辞：豁免只及仓库订阅与本地包注册表面，`vcc.liteDb` 等其余存储面与项目文件面
  维持禁止/只读）；权威行刷新至 1.4.0。
