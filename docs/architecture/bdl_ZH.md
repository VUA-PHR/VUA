...
`schemas/bdl-queries/`（查询契约，已升至 v0.3），实现位于 `crates/orchestrator` 的
`bdl_store` / `bdl_queries` 模块。访问面仍仅向 AMF 应用服务开放；后续公开读取面需另行接受
契约。实体身份、页面观察格式、条款表示与兼容证据的进一步演进随新的 AMF + BDL 垂直切片升版。
