...
> 2026-09-22 产品方向已更新，实现迁移尚未宣称完成。Recipe 仅承诺素材组合与明确选项；
> MA 能力条件和 SDK 检查由上游拥有，VUA 验证集成并报告自身失败。新手路径由 Wizard
> 选择。具体范围及未决项以 [产品边界](../product-boundary_ZH.md)「面向新手的制作范围」为准。
> Release 运行中条目不等于不可变 Build Record；后续契约接线保持此区别。
...
- 检查证据归入制作记录；通知中心与 Release 条目共享问题详情，不再要求独立 Inspection 页面；
...
2. **ALCOM 项目兼容**：只读识别 ALCOM 原项目；写操作仅作用于用户显式导入的 VUA 管理副本。
3. **VCC 项目兼容**：只读识别 VCC 原项目；写操作仅作用于用户显式导入的 VUA 管理副本。
...
- 1.2.0（2026-09-22）：对齐产品边界 1.5.0 的 Recipe/检查/上游职责；纠正旧 ALCOM/VCC 继续管理措辞，不改变既有 U3/U14 边界。
