...
| docs/protocols/packages-ops-v0.4_ZH.md | 0.4 | 已冻结（包管理 P3 写面 A4 仓库订阅增删词表行，2026-09-19：三方法一次冻结〔无 preview 对偶——远端网络段与无摘要可绑定双事实如实立面；添加面不宣称幂等——库面守卫拒绝如实上呈〕，双语协议本＋REGISTRY 登记随本冻结批；明确词面之外节载明启停候 W25 键名真机核实／重排候后续需求／凭据传输不收；诚实边界节载明词面尚未接线——wire 路由/served 行/信封组装候下一核心接线切片，接线前三方法在 wire 面不存在，信封常量候接线批 0.4.x 载明照 A3 先例） | 核心 | 2026-09-19 |
