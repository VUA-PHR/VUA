...
- Unity Bridge 入口和参数改为新的 VUA 命名，并与 `com.ph-r.vua` C# Package 同步；
...
结果：138 项自动测试通过，3 项需要真实机器或网络的手动测试被显式忽略；Clippy 零警告。
Unity Bridge v1 的请求/结果示例与 Rust 操作枚举也由契约测试共同校验。
